...
<div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
    <div class="rounded-xl bg-white p-6 shadow-sm">
        <h3 class="mb-5 font-semibold text-gray-800">Detail Soal</h3>
        <form method="POST" action="{{ route('admin.soal.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @include('admin.soal._form-fields')
            <div class="flex gap-3"><button class="flex-1 rounded-xl bg-blue-600 py-3 font-semibold text-white">Simpan Soal</button><a href="{{ route('admin.soal.index') }}" class="rounded-xl border px-5 py-3">Batal</a></div>
        </form>
    </div>
